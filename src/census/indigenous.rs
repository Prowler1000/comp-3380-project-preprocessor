...
pub fn get_indigenous(
    sheet: &Range<Data>,
) -> anyhow::Result<(IndigenousIdentity, IndigenousAncestry)> {
    Ok((
        get_indigenous_identity(sheet)?,
        get_indigenous_ancestry(sheet)?,
    ))
}
...
pub fn get_indigenous_identity(sheet: &Range<Data>) -> anyhow::Result<IndigenousIdentity> {
    ...
    Ok(IndigenousIdentity {
        métis: get_int_rounding(sheet.get_value((178, 1))).context("Invalid count for Métis single identity")? as u64,
        first_nations: get_int_rounding(sheet.get_value((179, 1))).context("Invalid count for First Nations (North American Indian) single identity")? as u64,
        inuk: get_int_rounding(sheet.get_value((180, 1))).context("Invalid count for Inuk (Inuit) single identity")? as u64,
        multiple: get_int_rounding(sheet.get_value((181, 1))).context("Invalid count for Multiple Indigenous identities")? as u64,
        other: get_int_rounding(sheet.get_value((182, 1))).context("Invalid count for Aboriginal identities not included elsewhere")? as u64,
    })
}
...
pub fn get_indigenous_ancestry(sheet: &Range<Data>) -> anyhow::Result<IndigenousAncestry> {
    ...
    Ok(IndigenousAncestry {
        first_nations: get_int_rounding(sheet.get_value((190, 1))).context("Invalid count for First Nations (North American Indian) Indigenous ancestry")? as u64,
        métis: get_int_rounding(sheet.get_value((191, 1))).context("Invalid count for Métis ancestry")? as u64,
        inuit: get_int_rounding(sheet.get_value((192, 1))).context("Invalid count for Inuit ancestry")? as u64,
    })
}
