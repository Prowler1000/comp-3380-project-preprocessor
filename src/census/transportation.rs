...
pub fn get_transportation(sheet: &Range<Data>) -> anyhow::Result<MainTransportation> {
    ...
    Ok(MainTransportation {
        personal_vehicle_driver: get_row_population(sheet, 476).unwrap(),
        public_transit: get_row_population(sheet, 477).unwrap(),
        personal_vehicle_passenger: get_row_population(sheet, 478).unwrap(),
        walk: get_row_population(sheet, 479).unwrap(),
        bicycle: get_row_population(sheet, 480).unwrap(),
        other: get_row_population(sheet, 481).unwrap(),
    })
}
